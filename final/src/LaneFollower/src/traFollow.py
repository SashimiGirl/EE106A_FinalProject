...
def control(x, y):
	global pub, go_right, searching, i
	K1 = 0.25
	K2 = -1.5
	if (searching):
		K1 = 0.12
	...
	try:
		...
		#calculate_position(multi[0], multi[1])
...
def calculate_position(vel, avel):
	global x_points, y_points, angle, i
	t = 0.2
	angle += avel*t
	print angle
	if (vel == 0):
		return
	y = vel*t * np.sin(angle)
	x = vel*t * np.cos(angle)

	x_points.append(x_points[i] + x)
	y_points.append(y_points[i] + y)
...
def listener():
	...
	rospy.Subscriber('/camera/rgb/image_color', Image, camera_data)
	rospy.spin()
